        ...
        if( entered_otp != expected_otp):
            ...
            if user_data:
                ...
                if user_data['role'] == 'admin':
                    user = models.Admin(admin_name=user_data['username'], email=user_data['email'], password=hashed_password)
                elif user_data['role'] == 'patient':
                    user = models.Patient(patient_name=user_data['username'], email=user_data['email'], password=hashed_password)
                elif user_data['role'] == 'doctor':
                    user = models.Doctor(doctor_name=user_data['username'], email=user_data['email'],
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            if role == 'admin':
                ...
                session['user_role'] = 'admin'  # Or the appropriate role for the user
                return redirect(url_for('Admin'))
            
            elif role == 'patient':
                session['user_id'] = user.id  # Replace 'user.id' with the actual ID of the logged-in user
                session['user_name']=user.patient_name
                session['user_role'] = 'patient'  # Or the appropriate role for the user
...
def book_appointment():
    ...
    if request.method == 'POST':
        ...
        appointment=models.Appointment(patient_name=patient_name,doctor_name=doctor_name,appointment_datetime=booking_datetime)
        db.session.add(appointment)
        db.session.commit()


    return render_template('book_appointment.html',specialities=specialities,doctors=doctors)
...
def Doctor():
    if 'user_id' in session and 'user_role' in session and session['user_role'] == 'doctor':
        doctor_name = session['user_name']
        # Fetch the doctor's appointments from the database
        appointments = models.Appointment.query.filter_by(doctor_name=doctor_name).all()
...
def download_reports():
    if 'user_id' in session and 'user_role' in session and session['user_role'] == 'doctor':
        ...
        # Create the doctor_downloads folder if it doesn't exist
        folder_path = 'doctor_downloads'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        # Create a CSV file and write the appointments data
        file_name = f'{doctor_name}_appointments_{selected_date}.csv'
        file_path = os.path.join(folder_path, file_name)
        with open(file_path, 'w', newline='') as csvfile:
            fieldnames = ['Patient', 'Date & Time', 'Status']
